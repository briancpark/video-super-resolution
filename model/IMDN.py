...
class IMDB_plus(nn.Module):
    def __init__(self, in_channels, distillation_rate=1 / 6):
        ...
        self.c2 = conv_layer(
            self.distilled_channels * 5, self.distilled_channels * 5, 3
        )  # 30 --> 30
        self.c3 = conv_layer(
            self.distilled_channels * 4, self.distilled_channels * 4, 3
        )  # 24 --> 24
        self.c4 = conv_layer(
            self.distilled_channels * 3, self.distilled_channels * 3, 3
        )  # 18 --> 18
        self.c5 = conv_layer(
            self.distilled_channels * 2, self.distilled_channels * 2, 3
        )  # 12 --> 12
        self.c6 = conv_layer(
            self.distilled_channels * 1, self.distilled_channels * 1, 3
        )  # 6 --> 6
        self.act = nn.SiLU()
    ...
    def forward(self, x):
        ...
        distilled_c1, remaining_c1 = torch.split(
            out_c1, (self.distilled_channels, self.distilled_channels * 5), dim=1
        )  # 6, 30
        out_c2 = self.act(self.c2(remaining_c1))  # 30 --> 30
        distilled_c2, remaining_c2 = torch.split(
            out_c2, (self.distilled_channels, self.distilled_channels * 4), dim=1
        )  # 6, 24
        out_c3 = self.act(self.c3(remaining_c2))  # 24 --> 24
        distilled_c3, remaining_c3 = torch.split(
            out_c3, (self.distilled_channels, self.distilled_channels * 3), dim=1
        )  # 6, 18
        out_c4 = self.act(self.c4(remaining_c3))  # 18 --> 18
        distilled_c4, remaining_c4 = torch.split(
            out_c4, (self.distilled_channels, self.distilled_channels * 2), dim=1
        )  # 6, 12
        out_c5 = self.act(self.c5(remaining_c4))  # 12 --> 12
        distilled_c5, remaining_c5 = torch.split(
            out_c5, (self.distilled_channels, self.distilled_channels), dim=1
        )  # 6, 6
